     ...
     +dftz,dstx,dsty,dstz,dsvx,dsvy,dsvz,dtvx,dtvy,dtvz,q,rati
      ...
      character adummy*150,ares,header*150
      character*4,allocatable:: atsym(:)

      open(12,file='tetrahedrals.dat')
      open(16,file='probdist_top.dat')
...
c do stuff until the end of the input *.gro
     ...
     +   (bin(j).eq.bin(nbins).and.z(i).ge.zbox)))then 
       ...
       tk = 0
       vk = 0
c f,s,t, and v are the 1st,2nd,3rd, and 4th closest waters 
c *k is the number of the closest waters
c *m is the distance from the closest waters
        do
        k = k + 1
        if(k.gt.iatoms) exit
c accounting for periodic boundry conditions
        if(atsym(k).eq.'OW'.and.k.ne.i) then
...
c if 3rd =< distance < 4th, set 4th = distance
         vm = d
         vk = k
        endif
...
c if the cosines are stupid numbers, it's counted as a fail
       fails = fails + 1
       endif
       endif
...
c rati helps determine if data points are reasonable
...
c I have never seen it write this line
c try increasing the cutoff for cycling, it was added to speed things up
      deallocate(bin,x,y,z,atsym,tq,waters,ran,prob)
      close(9)
      close(12)
      close(16)
!      close(23) 
