...
! gfortran catnip.f95 -Ofast -o catnip.e
      ...
      rngesus,ios,intnum,moldel,lugicul,brexit
      integer, allocatable:: groupies(:),molnum(:),atnum(:),nmolnum(:)& 
      ,natnum(:),catdel(:),tempint(:)
      character:: str*150,agroup*90,g1*4,g2*4,outbox*90,outtop*90,&
...
! translate it away
                  ...
                  a1=gridbox(rngesus,1)-tryxyz(l,1)
                  a2=gridbox(rngesus,2)-tryxyz(l,2)
                  a3=gridbox(rngesus,3)-tryxyz(l,3)
...
! if the atoms are on top of each other, use the x axis
                  ...
                  if(a.gt.0.001.and.b.gt.0.001) then
                    theta=sqrt((0.15**2-a-b)/(-2.*sqrt(a*b)))
...
! account for pbc, but don't worry about sign
